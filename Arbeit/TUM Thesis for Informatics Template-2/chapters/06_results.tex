...
In the unlabeled MMD-loss, the target labels are unknown. Therefore, the intra- and inter-class distance between source and target samples is minimized equally. Obviously, this increases the compactness but reduces the class separability in both domains. In the literature some approaches extend the unlabeled MMD-based model training with a distance-based optimization, which includes target labels \cite{Pandhare2021}. Usually those approaches restrict the supervised training to the source domain. The equal training conditions allow a comparison of those approaches with the unlabeled MMD-based optimization. Including target labels in the MMD-loss reveals the deficits of the unlabeled MMD-loss. Anyhow, those methods are impractical for the real-world use. The goal of domain adaption approaches is to transfer neural networks trained on the source domain data to the target domain. If that requires target domain samples one could train the neural network straight away on the target domain. Besides that, these approaches expect the time consuming manual labeling of the target domain. The target label requirement prevents the advantage of domain adaption approaches, which optimize the models on a known labeled dataset and adjust it to various different tasks by including the corresponding unlabeled dataset in the training. In this section a labeled MMD-loss is presented which includes target labels but restricts the supervised training to the source domain. The distance between source and target samples of similar and different classes is optimized separately. The labeled MMD-loss minimizes the intra-class distance and maximizes the inter-class distance for both domains. This separate consideration of class distances allows a simultaneous improvement of class separability and compactness. A weighted total loss is applied which combines the source CE-loss, the intra-class and the inter-class MMD-loss. This approach expects additional hyperparameter, which need to be defined beforehand. The hyperparameters GAMMA\_Intra\_Class and GAMMA\_Inter\_Class are used to balance the training scope of maximizing the intra-class distance and minimizing the inter-class distance and source CE-loss:
...
This section analyzes the influence of applying the MMD-loss in different latent feature maps of the CNN and classifier. Inspired by the computer vision community especially the domain discrepancy reduction in the feature extractor layers is of great interest. Two approaches were evaluated which are specified in table \ref{tab:MMD_layer_choice_dummy}
...
\begin{tabular}{llllllll}
  \toprule
  Model          & Conv1 & Conv2 & Conv3 & Flattend & FC1 & FC2 \\
  \midrule
...
\vspace{.5cm}
 ...
 \parbox[t]{0mm}{\multirow{1}{*}{\rotatebox[origin=c]{90}{\thead{FUL \\ MMD}}}} & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark\\
...
\end {table}
 ...
 The accuracy, the source CE and MMD-loss during the training process are shown in fig. \ref{fig:accuracy_cnn_and_no_cnn_mmd} and fig. \ref{fig:loss_cnn_and_no_cnn_mmd}. The accuracies in source and target domain are higher when including the CNN feature maps into the MMD-loss. Besides that, including the CNN feature maps in the MMD-loss increases the stability of the training. The losses, as well as the accuracies, converge faster and smoother.
