...
source domain $X^{s}$ and target domain $X^{t}$ and $F(\cdot)$ is the transformation of the input samples to its feature representation in the current fully connected layer. Generally, the goal of the discriminators is to identify the corresponding domain of each sample and of the feature mapper to extract features which are domain independent, which do not allow the discriminator to predict the corresponding domain. To satisfy the Lipschitz constraint condition of Wasserstein distance an additional gradient penalty is applied: 
 ...
 \end{equation}
 where $P_{\tilde{x}}$ is a distribution of samples coming from the line connecting a pair of points sampled from the source and target domain. In the adversarial learning the feature mapper and discriminators are optimized in an alternating procedure. Firstly, the domain critics are optimized to minimize and afterwards the feature mapper are tuned to maximize the Wasserstein distance extended with the gradient penalty. The adversial training can therefore be described as following: 
 \begin{equation}
    \min_{\theta_{f}} \max_{\theta_{d}} (L_{wd} - \lambda L_{gp}), 
 \end{equation}
 where $\theta_{f}$ and $\theta_{d}$ are the parameters of the feature mapper and domain critics, and $\lambda$ is the penalty coefficient. The workflow of the model is again described in fig. \ref{fig:WDMAN_workflow}
 \begin{figure}[p]
  \centering
