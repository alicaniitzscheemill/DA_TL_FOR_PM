...
A Deep Belief Network(DBN) is a generative model that generates depth by stacking of Restricted Boltzmann machines (RBMs), which are widely used in research and application. Since DBMs are generative model they are used in an unsupervised or a supervised manner, which means, they are able to do feature learning/extraction and classification. RBMs consits of two groups of nodes where one is defined as hidden and the other as visible unit. The visible layer stores information about the input into the network and is therefore known. The hidden layer represents in the input in a new and feature space and is therefore unknown. DBNs use Restricted Boltzmann machines (RBMs), which are a variation of regular Boltzmann machines with the restriction that it's units must form a bipartite graph. This requires that all RBM units are separated in two disjoint and independent subsets (hidden and visible subset). The visible and hidden units are connected through weighted connections but not the units belonging to the same layer. The units in the hidden and visible layer are binary. The matrix $W \in \mathbb{R}^{mxn}$ contains all the weights for connecting the m nodes of the hidden and n nodes of visible unit. $a \in \mathbb{R}^{m}$ and $b \in \mathbb{R}^{n}$ contains the biases of the hidden and visible unit \cite{Hinton2010}. The connection of the visible and hidden unit is visualized in \ref{fig:RBM}.
...
\end{aligned}
\end{equation}
This iterative process of reevaluating the units in the hidden and visible layer with the eq. \ref{eq:prob_hidden_layer} and \ref{eq:prob_reconstructed_visible_layer} is also visualized in fig. \ref{fig:RBM}. The weights and biases of the network can be updated according to the reconstruction error:
...
\end{aligned}
\end{equation}
where the angle brackets denote expectations under the distribution specified in the subscripts. The subscript $data$ refers to the combination of hidden visible vectors $\mathbf{v,h}$ after the update of the hidden layer with eq. \ref{eq:prob_hidden_layer} and the subscript $reconstructed$ to those after the reconstruction of the visible units with eq. \ref{eq:prob_reconstructed_visible_layer}. A better training is achieved if more steps of alternating updating the visible and hidden layer are performed before collecting the statistics for the second term $\langle v_{i}, h_{j}\rangle_{reconstructed}$ in the learning rule \cite{Hinton2010}.

RBMs are generative models which make guesses about the probability distribution of the original input. By training the RBM the divergence between the true input distribution and the reconstructed ones is minimized \cite{Hinton2010}. Stacking several RBMs in DBN as visualized in \ref{fig:DBN} generates an even more powerful model. 
...
\begin{figure}[htpb]
  ...
  \caption {Stacking RBMs to construct a DBN} \label{fig:DBN}
\end{figure}
\FloatBarrier 
