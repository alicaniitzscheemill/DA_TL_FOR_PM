...
def plot_distribution():
    ...
    for element in csv_file_path_folder_elements:
        ...
        for i in range(4):
            ax.scatter(df[columns[0+i*3]], df[columns[1+i*3]], df[columns[2+i*3]], marker=m[i], label=data_label[i])
        ...
        ax.set_zlabel('Neuron 3 $\longrightarrow$', rotation=0, labelpad=70, size=45)

        ax.tick_params(axis="x", pad=20, labelsize= 27)
        ax.tick_params(axis="y", pad=20, labelsize= 27)
        ax.tick_params(axis="z", pad=30, labelsize= 27)
        ...
        #legend = ax.legend(prop={'size': 32}, markerscale=4, loc='upper right')
