...
def concatenate_data_from_BSD_state(data_folders, data_path, features_of_interest, window_size, overlap_size):
    """
    ...
    for BSD_path in data_folders.keys(): #folder path
        for file_path in data_folders[BSD_path]: #file path 
            ...
            data_BSD_file = split_data(data_BSD_file, window_size, overlap_size) #window the data
...
class TimeSeriesData(Dataset):
    ...
    def __init__(self, data_path, list_of_BSD_states, window_size, overlap_size, features_of_interest):
        ...
        self.n_samples, self.x_data, self.y_data = concatenate_data_from_BSD_state(data_folders, data_path, features_of_interest, window_size, overlap_size)
...
def create_dataloader(data_path, list_of_BSD_states, window_size, overlap_size, features_of_interest, dataloader_split_ce, dataloader_split_mmd, dataloader_split_val, batch_size):
    """
    ...
    #get dataset
    dataset = TimeSeriesData(data_path, list_of_BSD_states, window_size, overlap_size, features_of_interest)

    # define train/val dimensions
    dataset_size_ce= int(dataloader_split_ce * len(dataset))
    dataset_size_mmd= int(dataloader_split_mmd * len(dataset))
    dataset_size_val= int(dataloader_split_val * len(dataset))
